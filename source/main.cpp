...
inline void button_game(bool *isDon, bool *isKatsu, OPTION_T Option, unsigned int key)
{
	...
	for (int i = 0; i < numKeys; ++i)
	{
		if (key & keys[i])
		{
			if (Optionkeys[i] == KEY_DON)
				*isDon = true;
			else if (Optionkeys[i] == KEY_KATSU)
				*isKatsu = true;
		}
	}
}
...
int main()
{
	...
	while (aptMainLoop())
	{
		...
		case SCENE_SELECTLOAD: // ロード画面
			...
			if (don_cnt > 0)
				C2D_DrawEllipseSolid(55, 30, 0, 210, 210, C2D_Color32(0xF7, 0x4A, 0x21, 0x7F));
		...
		case SCENE_MAINLOAD: // ロード中
			...
			if (!SelectedSong.course_exist[course])
				load_tja_notes(-1, SelectedSong);
			else

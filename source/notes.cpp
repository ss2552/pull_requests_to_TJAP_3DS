...
void notes_main(bool isDon, bool isKatsu, char tja_notes[MEASURE_MAX][NOTES_MEASURE_MAX], MEASURE_T Measure[MEASURE_MAX], int cnt, C2D_Sprite sprites[SPRITES_NUMER])
{
	...
	if (cnt >= 0 && isNotesLoad)
	{
		...
		while (Measure[MeasureCount].create_time <= CurrentTimeNotes && !Branch.wait)
		{
			...
			if (isNotesLoad == false || Branch.wait)
				break;
			...
			for (int i = 0; i < NotesCount; ++i)
			{
				...
				if (id != -1 && ctoi(tja_notes[Measure[MeasureCount].notes][i]) != 0 && Measure[MeasureCount].branch == Branch.course)
				{
					...
					if ((knd == NOTES_ROLL || knd == NOTES_BIGROLL || knd == NOTES_BALLOON) && (PreNotesKnd == knd))
					{ // 55558のような表記に対応
						continue;
					}
	...
	for (int i = 0, j = BARLINE_MAX - 1; i < j; ++i)
	{
		...
		if (BarLine[i].flag)
		{
			...
			if ((BarLine[i].x < 62 && BarLine[i].scroll > 0) || (BarLine[i].x > 400 && BarLine[i].scroll < 0))
				BarLine[i].flag = false;
		}
	}

	if (!get_isPause())
		notes_calc(isDon, isKatsu, bpm, CurrentTimeNotes, cnt, sprites, Measure);
	if (!Option.isStelth)
		notes_draw(sprites);
	draw_emblem(sprites);
	draw_judge(CurrentTimeNotes, sprites);

	if (MaxMeasureCount < MeasureCount)
		MaxMeasureCount = MeasureCount;
		...
		if ((Measure[i].branch == Branch.course || Measure[i].branch == -1) && Measure[i].flag)
		{
			...
			if (Measure[i].command != -1 && Measure[i].judge_time <= CurrentTimeNotes)
			{
				...
				case COMMAND_BRANCHSTART:
					...
					if (isLevelHold == false && (Branch.knd != 0 || JudgeRollState == -1))
					{ // 連打分岐の時は連打が無くなってから分岐
			...
			if (NotFalse == false && Measure[i].judge_time <= CurrentTimeNotes)
...
void draw_judge(double CurrentTimeNotes, C2D_Sprite sprites[SPRITES_NUMER])
{
	...
	if (isJudgeDisp)
	{
		...
		if (CurrentTimeNotes - JudgeMakeTime >= 0.5)
			isJudgeDisp = false;
	}
}
...
inline void notes_judge(double CurrentTimeNotes, bool isDon, bool isKatsu, int cnt, int branch)
{
	...
	for (int i = 0, j = ROLL_MAX - 1; i < j; ++i)
	{
		...
		if (RollNotes[i].flag &&
			...
			(RollNotes[i].end_id == -1 || (RollNotes[i].end_id != -1 && Notes[RollNotes[i].end_id].judge_time > CurrentTimeNotes)))
			JudgeRollState = RollNotes[i].knd;
	}
	...
	if (JBS != JudgeBalloonState && JudgeBalloonState != -1)
	{
		...
		if (balloon[branch][BalloonCount[branch]] != 0)
			BalloonNotes[JudgeBalloonState].need_hit = balloon[branch][BalloonCount[branch]];
		else
			BalloonNotes[JudgeBalloonState].need_hit = 5;
		if (branch == 0)
			++BalloonCount[0];
		else
		{
	...
	{ // オート
		...
		for (int i = 0, j = Notes.size() - 1; i < j; ++i)
		{
				...
				Notes[i].isThrough == false && Notes[i].knd < NOTES_ROLL)
			{
				...
				if (Notes[i].knd == NOTES_BIGDON || Notes[i].knd == NOTES_BIGKATSU)
					update_score(SPECIAL_PERFECT);
				else if (Notes[i].knd == NOTES_DON || Notes[i].knd == NOTES_KATSU)
	...
	{ // 手動
		...
		bool isBig;
		if (Notes[CurrentJudgeNotes[0]].knd == NOTES_BIGDON || Notes[CurrentJudgeNotes[0]].knd == NOTES_BIGKATSU)
			isBig = true;
		else
			isBig = false;

		if (isDon && CurrentJudgeNotes[0] != -1)
		{ // ドン

			if (CurrentJudgeNotesLag[0] <= Option.judge_range_perfect)
			{ // 良
			...
			else if (CurrentJudgeNotesLag[0] <= Option.judge_range_nice)
			{ // 可
			...
			else if (CurrentJudgeNotesLag[0] <= Option.judge_range_bad)
			{ // 不可
		...
		if (isKatsu && CurrentJudgeNotes[1] != -1)
		{ // カツ

			if (CurrentJudgeNotesLag[1] <= Option.judge_range_perfect)
			{ // 良
			...
			else if (CurrentJudgeNotesLag[1] <= Option.judge_range_nice)
			{ // 可
				make_judge(1, CurrentTimeNotes);
				delete_notes(CurrentJudgeNotes[1]);
				if (isBig)
			...
			else if (CurrentJudgeNotesLag[1] <= Option.judge_range_bad)
			{ // 不可
				make_judge(BAD, CurrentTimeNotes);
				delete_notes(CurrentJudgeNotes[1]);
				update_score(BAD);
	...
	if (JudgeBalloonState != -1 && BalloonNotes[JudgeBalloonState].current_hit >= BalloonNotes[JudgeBalloonState].need_hit)
	{
		...
		if (BalloonNotes[JudgeBalloonState].end_id != -1)
			delete_notes(BalloonNotes[JudgeBalloonState].end_id);
		else
			delete_notes(BalloonNotes[JudgeBalloonState].start_id);
...
void notes_calc(bool isDon, bool isKatsu, double bpm, double CurrentTimeNotes, int cnt, C2D_Sprite sprites[SPRITES_NUMER], MEASURE_T Measure[MEASURE_MAX])
{
	...
	{ // 計算
		...
		if (Notes[i].flag)
		{
			...
			case NOTES_BALLOON:
				if ((Notes[i].x <= NOTES_JUDGE_X && Notes[i].scroll > 0) || (Notes[i].x >= NOTES_JUDGE_X && Notes[i].scroll < 0))
	...
	{ // 連打のバグ回避のためノーツの削除は一番最後
			...
			Notes[i].knd != NOTES_ROLL && Notes[i].knd != NOTES_BIGROLL)
		{
				...
				(Notes[i].knd > NOTES_REST && Notes[i].knd < NOTES_ROLL))
			{
				...
				{ // オート時はスルー以外良判定に
					if (Notes[i].knd == NOTES_DON || Notes[i].knd == NOTES_KATSU)
						update_score(PERFECT);
					else if (Notes[i].knd == NOTES_BIGDON || Notes[i].knd == NOTES_BIGKATSU)
						update_score(SPECIAL_PERFECT);
					if (Notes[i].knd == NOTES_DON || Notes[i].knd == NOTES_BIGDON)
						play_sound(SOUND_DON);
					if (Notes[i].knd == NOTES_KATSU || Notes[i].knd == NOTES_BIGKATSU)
						play_sound(SOUND_KATSU);
				}
			}
			delete_notes(i);
		...
		if (Notes[i].flag)
		{
			...
			case NOTES_ROLL:
				...
				if (RollNotes[Notes[i].roll_id].flag)
				{
					...
					double end_x;
					if (RollNotes[Notes[i].roll_id].end_id == -1 || RollNotes[Notes[i].roll_id].end_x >= 420.0f)
						end_x = TOP_WIDTH + 20.0f;
					else
						end_x = RollNotes[Notes[i].roll_id].end_x;
			...
			case NOTES_BALLOON:
				...
				if (BalloonNotes[Notes[i].roll_id].current_hit >= 1)
...
int notes_cmp(const void *p, const void *q)
{ // 比較用
...
void delete_roll(int i)
{
	...
	if (i >= 0 && i < ROLL_MAX)
	{
		if (RollNotes[i].start_id != -1 && Notes[RollNotes[i].start_id].flag)
			delete_notes(RollNotes[i].start_id);
		RollNotes[i].id = -1;
...
static int find_roll_end_id()
{ // startの値だけ入ってる連打idを返す
...
int find_balloon_end_id()
{ // startの値だけ入ってる風船idを返す
...
void delete_notes(int i)
{
	...
	if (i >= 0 &&
		Notes[i].roll_id != -1 &&
		BalloonNotes[Notes[i].roll_id].flag)
	{ // 風船削除

		if (Notes[i].knd == NOTES_BALLOONEND)
		{

			if (BalloonNotes[Notes[i].roll_id].start_id != -1)
...
bool get_notes_finish()
{
	...
	for (int i = 0, j = Notes.size() - 1; i < j; ++i)
	{
		...
		if (Notes[i].flag && !Notes[i].isThrough)
			return false;
	}
	return true;
}
...
void draw_condition()
{
	...
	for (int j = 0; j < 4; ++j)
	{
		...
		tx = 0;
		if (strcmp(exam[j][0], "jb") == 0)
			draw_condition_text(50, 154 + 20 * j, Text[get_lang()][TEXT_NUM_BAD], &width, &height);
		else if (strcmp(exam[j][0], "jg") == 0)
			draw_condition_text(50, 154 + 20 * j, Text[get_lang()][TEXT_NUM_NICE], &width, &height);
		else if (strcmp(exam[j][0], "jp") == 0)
			draw_condition_text(50, 154 + 20 * j, Text[get_lang()][TEXT_NUM_PERFECT], &width, &height);
		else if (strcmp(exam[j][0], "s") == 0)
			draw_condition_text(50, 154 + 20 * j, Text[get_lang()][TEXT_NUM_SCORE], &width, &height);
		else if (strcmp(exam[j][0], "r") == 0)
			draw_condition_text(50, 154 + 20 * j, Text[get_lang()][TEXT_NUM_ROLL], &width, &height);
		else if (strcmp(exam[j][0], "h") == 0)
			draw_condition_text(50, 154 + 20 * j, Text[get_lang()][TEXT_NUM_HIT], &width, &height);
		else if (strcmp(exam[j][0], "g") == 0)
			draw_condition_text(50, 154 + 20 * j, Text[get_lang()][TEXT_NUM_GAUGE], &width, &height);
		tx += width;
		...
		if (strcmp(exam[j][3], "m") == 0)
			draw_condition_text(50 + tx, 154 + 20 * j, Text[get_lang()][TEXT_NUM_UP], &width, &height);
		else if (strcmp(exam[j][3], "l") == 0)
			draw_condition_text(50 + tx, 154 + 20 * j, Text[get_lang()][TEXT_NUM_DOWN], &width, &height);
	}
}

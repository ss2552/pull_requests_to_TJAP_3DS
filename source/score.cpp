...
void update_score(int knd)
{
	...
	{ // 新配点
		if (0 <= combo && combo < 9)
			DiffMul = 0;
		else if (9 <= combo && combo < 29)
			DiffMul = 1;
		else if (29 <= combo && combo < 49)
			DiffMul = 2;
		else if (49 <= combo && combo < 99)
			DiffMul = 4;
		else if (99 <= combo)
			DiffMul = 8;
	}
	...
	if ((TotalPerfectCount + TotalNiceCount + TotalBadCount) != 0)
		Precision = (double)TotalPerfectCount / (TotalPerfectCount + TotalNiceCount + TotalBadCount) * 100.0;
	else
		Precision = 0;
	if ((CurrentPerfectCount + CurrentNiceCount + CurrentBadCount) != 0)
		CurrentPrecision = (double)CurrentPerfectCount / (CurrentPerfectCount + CurrentNiceCount + CurrentBadCount) * 100.0;
	else
		CurrentPrecision = 0;
}
...
void draw_score(C2D_Sprite sprites[SPRITES_NUMER])
{
	...
	for (int i = 0; i < 7; ++i)
	{
		...
		if (TotalScore / pow(10, i) > 0)
		{
			int n = TotalScore / pow(10, i) % 10;
	...
	for (int i = 0; i < 5; ++i)
	{
		...
		if (combo >= 10 && combo / pow(10, i) > 0)
		{

			int n = combo / pow(10, i) % 10;
	...
	for (j = 0; j < 4; ++j)
	{
		if (CurrentRollCount / pow(10, j) == 0)
			break;
	}
	...
	for (int i = 0; i < 4; ++i)
	{
		...
		if (CurrentRollCount / pow(10, i) > 0)
		{

			int n = CurrentRollCount / pow(10, i) % 10;
	...
	for (j = 0; j < 5; ++j)
	{
		if (CurrentBalloonCount / pow(10, j) == 0)
			break;
	}
	...
	for (int i = 0; i < 5; ++i)
	{
		...
		if (CurrentBalloonCount / pow(10, i) > 0)
		{

			int n = CurrentBalloonCount / pow(10, i) % 10;
...
void draw_gauge(C2D_Sprite sprites[SPRITES_NUMER])
{
	...
	if (Gauge.score > Gauge.soul)
		Gauge.score = Gauge.soul;
	if (gauge > 1.0)
		gauge = 1.0;
	...
	// 魂
	for (int i = 0; i < 2; ++i)
		C2D_SpriteSetPos(&sprites[SPRITE_SOUL_ON + i], 385, 75);
	...
	if ((Gauge.score / 200) * 200 >= Gauge.soul)
	{
		...
		C2D_DrawImage(sprites[SPRITE_SOUL_OFF].image, &sprites[SPRITE_SOUL_OFF].params, NULL);
}
...
void draw_gauge_result(C2D_Sprite sprites[SPRITES_NUMER])
{
	...
	// 魂
	for (int i = 0; i < 2; ++i)
		C2D_SpriteSetPos(&sprites[SPRITE_SOUL_ON + i], 385 - diff * 2, 75);
...
{ // 分岐
	...
	{
	case 0: // 連打
		if (y <= CurrentTotalRollCount)
			branch = COMMAND_M;
		else if (x <= CurrentTotalRollCount)
			branch = COMMAND_E;
		else
			branch = COMMAND_N;
		break;
	case 1: // 精度
		if (y <= CurrentPrecision)
			branch = COMMAND_M;
		else if (x <= CurrentPrecision)
			branch = COMMAND_E;
		else
			branch = COMMAND_N;
		break;
	case 2: // スコア
		if (y <= CurrentScore)
			branch = COMMAND_M;
		else if (x <= CurrentScore)
			branch = COMMAND_E;
		else
			branch = COMMAND_N;
		break;
	case 3: // 大音符
		if (y <= CurrentDaiNotes)
			branch = COMMAND_M;
		else if (x <= CurrentDaiNotes)
			branch = COMMAND_E;
		else
			branch = COMMAND_N;
		break;
...
int dan_condition()
{
	...
	for (int i = 0; i < 7; ++i)
	{
		if ((strcmp(exam[0][0], types[i]) == 0 && strcmp(exam[0][3], "l") == 0 && counts[i] >= redCdn[0]) ||
			...
			(strcmp(exam[3][0], types[i]) == 0 && strcmp(exam[3][3], "l") == 0 && counts[i] >= redCdn[3]))
			++isBadCondition;
	}

	if (get_notes_finish() == true)
	{ // 条件に以上(例:exam[3] = "m")がある場合、曲が終わるまで判定しない
		for (int i = 0; i < 7; ++i)
		{
			if ((strcmp(exam[0][0], types[i]) == 0 && strcmp(exam[0][3], "m") == 0 && counts[i] < redCdn[0]) ||
				...
				(strcmp(exam[3][0], types[i]) == 0 && strcmp(exam[3][3], "m") == 0 && counts[i] < redCdn[3]))
				++isBadCondition;
		}
	}
...
void calc_base_score(MEASURE_T Measure[MEASURE_MAX], char notes[MEASURE_MAX][NOTES_MEASURE_MAX])
{ // 初項と公差を計算　魂ゲージの伸びも
	...
	case 5: // 太鼓タワー
		if (level == 10)
			BaseCeilingPoint = 1200000;
		else
			BaseCeilingPoint = 650000 + level * 50000;
		break;
	case 6: // 段位道場
	...
	while (isEND == false && i < MEASURE_MAX && Measure[i].flag == true)
	{ // 小節
		...
		if (Measure[i].firstmeasure != -1)
			NotesCountMax = Measure[Measure[i].firstmeasure].max_notes;
		else
		...
		{ // ノーツ
			...
			if (knd != 0)
			{
				...
				{
					if (knd == NOTES_BIGDON || knd == NOTES_BIGKATSU)
						special = 2.0;
					else
						special = 1.0;
					++combo;
					...
					{ // 新配点
						...
						if (combo >= 1 && combo <= 9)
							DiffTmp = 0;
						else if (combo >= 10 && combo <= 29)
							DiffTmp = 1;
						else if (combo >= 30 && combo <= 49)
							DiffTmp = 2;
				...
				else if (knd == NOTES_ROLLEND)
				{
					...
					if (roll_start_time != 0)
					{
						...
						if (RollKnd == NOTES_ROLL)
						{
							...
							{
								if (gogo == true)
									TmpBaseCeilingPoint -= RollCnt * 360;
								else
									TmpBaseCeilingPoint -= RollCnt * 300;
							}
							if (scoremode == 2)
							{
								if (gogo == true)
									TmpBaseCeilingPoint -= RollCnt * 120;
								else
						...
						else if (RollKnd == NOTES_BIGROLL)
						{
							...
							{
								if (gogo == true)
									TmpBaseCeilingPoint -= RollCnt * 430 * gogo;
								else
							...
							{
								if (gogo == true)
									TmpBaseCeilingPoint -= RollCnt * 240 * gogo;
								else
	...
	if ((TJA_Header.scoreinit == -1 || TJA_Header.scorediff == -1) && (scoremode == 1 || scoremode == 2))
	{ // 新配点と旧配点
	...
	case 6: // 段位道場
		...
		if (strcmp(exam[0][0], "g") == 0 && strcmp(exam[0][3], "m") == 0)
			Gauge.norma = Gauge.soul * (redCdn[0] / 100.00);
		else if (strcmp(exam[1][0], "g") == 0 && strcmp(exam[1][3], "m") == 0)
			Gauge.norma = Gauge.soul * (redCdn[1] / 100.00);
		else if (strcmp(exam[2][0], "g") == 0 && strcmp(exam[2][3], "m") == 0)
			Gauge.norma = Gauge.soul * (redCdn[2] / 100.00);
		else if (strcmp(exam[3][0], "g") == 0 && strcmp(exam[3][3], "m") == 0)
			Gauge.norma = Gauge.soul * (redCdn[3] / 100.00);
		break;
	}
	// init_score_after();
}

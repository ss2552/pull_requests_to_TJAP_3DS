		...
		while (fgets(buf, 128, fp) != NULL)
		{
			...
			if (strstr(buf, "SUBTITLE:") == buf)
			{
				if (buf[9] != '\n' && buf[9] != '\r')
				{
					...
					if (strstr(temp, "--") == temp)
						...
						Current_Header.subtitle_state = 2;
					else
			...
			if (strstr(buf, "COURSE:") == buf)
			{
				if (buf[7] != '\n' && buf[7] != '\r')
				{
					strlcpy(temp, buf + 7, strlen(buf) - 8);
					if (strlen(temp) == 1)
						Current_Header.course = atoi(temp); // 数字表記
					else if (strcmp(temp, "Easy") == 0 || strcmp(temp, "easy") == 0)
						course = COURSE_EASY; // 文字表記
					else if (strcmp(temp, "Normal") == 0 || strcmp(temp, "normal") == 0)
						course = COURSE_NORMAL;
					else if (strcmp(temp, "Hard") == 0 || strcmp(temp, "hard") == 0)
						course = COURSE_HARD;
					else if (strcmp(temp, "Oni") == 0 || strcmp(temp, "oni") == 0)
						course = COURSE_ONI;
					else if (strcmp(temp, "Edit") == 0 || strcmp(temp, "edit") == 0)
						course = COURSE_EDIT;
					else if (strcmp(temp, "Tower") == 0 || strcmp(temp, "tower") == 0)
						course = COURSE_TOWER;
					else if (strcmp(temp, "Dan") == 0 || strcmp(temp, "dan") == 0)
						course = COURSE_DAN;
...
void sort_measure_insertion(MEASURE_T t[], int array_size)
{ // create_timeでソート
...
{ // 最初に到達する小節の所要時間を計算
	...
	for (int i = 0; i < MEASURE_MAX; ++i)
	{
		...
		if (Measure[i].flag && Measure[i].command == -1)
		{
			...
			if (Measure[i].judge_time < Measure[tmp].judge_time)
				tmp = i;
		}
	}
	stme = tmp;
	...
	if ((fp = fopen(Song.tja, "r")) != NULL)
	{
			...
			isEnd == false)
		{
			...
			if (strstr(tja_notes[tja_cnt], "COURSE:") == tja_notes[tja_cnt])
			{
				...
				if (strlen(temp) == 1)
					CurrentCourse = atoi(temp); // 数字表記
				else if (strcmp(temp, "Easy") == 0 || strcmp(temp, "easy") == 0)
					CurrentCourse = COURSE_EASY; // 文字表記
				else if (strcmp(temp, "Normal") == 0 || strcmp(temp, "normal") == 0)
					CurrentCourse = COURSE_NORMAL;
				else if (strcmp(temp, "Hard") == 0 || strcmp(temp, "hard") == 0)
					CurrentCourse = COURSE_HARD;
				else if (strcmp(temp, "Oni") == 0 || strcmp(temp, "oni") == 0)
					CurrentCourse = COURSE_ONI;
				else if (strcmp(temp, "Edit") == 0 || strcmp(temp, "edit") == 0)
					CurrentCourse = COURSE_EDIT;
				else if (strcmp(temp, "Tower") == 0 || strcmp(temp, "tower") == 0)
					CurrentCourse = COURSE_TOWER;
				else if (strcmp(temp, "Dan") == 0 || strcmp(temp, "dan") == 0)
					CurrentCourse = COURSE_DAN;
			...
			if (isStart && isCourseMatch)
			{
				...
				else
				{
					...
					{ // 複数小節
						...
						int i = 0;
						while (tja_notes[tja_cnt][i] != '\n' && tja_notes[tja_cnt][i] != ',' && tja_notes[tja_cnt][i] != '/')
							++i;
						NotesCount += i - 1;
						if (tja_notes[tja_cnt][i] == '/')
							++NotesCount;
						if (tja_notes[tja_cnt][i] != ',' && tja_notes[tja_cnt][i] != '/')
							--i;
				...
				if (tja_notes[tja_cnt][0] == '#')
				{
					...
					case COMMAND_BRANCHSTART:
						...
						if (tja_cnt == 0)
							Measure[MeasureCount].judge_time = 0; // ノーツの前に分岐はすぐに判定
						break;
					case COMMAND_M:
					case COMMAND_E:
					case COMMAND_N:
				...
				if (isNoComma == false && NotesCount != 0 && tja_notes[tja_cnt][0] != '#')
				{ // 複数行小節の最後の行
					...
					for (int i = 1, j = MeasureCount - Measure[MeasureCount].firstmeasure + 1; i < j; ++i)
					{ // judge_timeの調整
						...
						{ // 複数行小節の最初の小節以外
							...
							Measure[Measure[MeasureCount].firstmeasure + i].judge_time =
								...
								(240.0 / Measure[Measure[MeasureCount].firstmeasure + i - 1].bpm * Measure[Measure[MeasureCount].firstmeasure + i - 1].measure) * Measure[Measure[MeasureCount].firstmeasure + i - 1].notes_count / Measure[Measure[MeasureCount].firstmeasure].max_notes; // delayはとりあえず放置
		...
		{ // 次の小節の判定時に発動する命令の調整
			...
			case COMMAND_GOGOEND:
				int n = i + 1;
				while (n <= MeasureMaxNumber && tja_notes[n][0] == '#')
					++n;
	...
	if (buf[0] == '#')
	{
		...
		{ // 値なし
			...
			// コメントなし 改行あり
				...
				strlcpy(command, buf + 1, length - 2);
			// コメントなし　改行なし
			else
				strlcpy(command, buf + 1, length);
		...
		else if (strcmp(command, "GOGOSTART") == 0)
			Command->knd = COMMAND_GOGOSTART;
		else if (strcmp(command, "GOGOEND") == 0)
		...
		else if (strcmp(command, "BRANCHEND") == 0)
		...
		else if (strcmp(command, "LEVELHOLD") == 0)
			Command->knd = COMMAND_LEVELHOLD;
		else if (strcmp(command, "BARLINEOFF") == 0)
			Command->knd = COMMAND_BARLINEOFF;
		else if (strcmp(command, "BARLINEON") == 0)
